...
export const ReportRenderer: React.FC<ReportRendererProps> = ({
  ...
  const markdownComponents = {
    ...
    table: ({ children }: any) => <CustomTable>{children}</CustomTable>,

    thead: ({ children }: any) => (
      <thead className="bg-[#141a24] text-slate-200 border-b border-white/[0.08] font-semibold text-xs">
        {children}
      </thead>
    ),
    ...
    code: ({ node, inline, className, children, ...props }: any) => {
      ...
      if (!inline) {
        ...
        return (
          <div className="my-4 rounded-xl border border-white/[0.08] bg-[#0c1017] overflow-hidden select-text">
            <div className="px-4 py-2 bg-[#121620] border-b border-white/[0.06] flex items-center justify-between text-xs text-slate-400 select-none">
              <span className="font-mono text-[11px] text-[#2dd4bf] uppercase">{languageName || 'code'}</span>
              <button
                type="button"
            ...
            <pre className="p-4 overflow-x-auto text-xs text-slate-200 font-mono leading-relaxed bg-[#07090e]/70">
              <code>{rawCode}</code>
            </pre>
          </div>
        );
      ...
      return (
        <code className="px-1.5 py-0.5 rounded bg-white/[0.06] text-[#2dd4bf] font-mono text-[12px] border border-white/[0.06]" {...props}>
          {children}
        </code>
      );
    },
  ...
  return (
    <div className="space-y-6 select-text">
      ...
      <div className="py-2.5 px-3.5 rounded-xl bg-white/[0.02] border border-white/[0.05] flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400 select-none">
        ...
        {headings.length > 0 && (
          ...
          >
            <ListTree className="w-3.5 h-3.5 text-[#2dd4bf]" />
            <span>{isArabic ? 'فهرس المحتويات' : 'Table of Contents'}</span>
            <span className="px-1.5 py-0.2 rounded-full bg-[#2dd4bf]/10 text-[#2dd4bf] text-[10px] font-mono">
              {headings.length}
            </span>
          </button>
        )}
      ...
      {showToc && headings.length > 0 && (
        <div className="p-4 rounded-xl bg-[#111620] border border-white/[0.08] shadow-2xl space-y-2 animate-fadeIn select-none">
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.06]">
            <h4 className="text-xs font-semibold text-slate-200 flex items-center gap-1.5">
              <ListTree className="w-4 h-4 text-[#2dd4bf]" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-56 overflow-y-auto custom-scrollbar pt-1">
            {headings.map((h, i) => (
              <button
                ...
                className={`text-left rtl:text-right p-2 rounded-lg text-xs hover:bg-white/[0.04] text-slate-300 hover:text-[#2dd4bf] transition truncate flex items-center gap-2 ${
      ...
      <article className="prose prose-invert max-w-none text-slate-300 text-[14.5px] leading-relaxed">
