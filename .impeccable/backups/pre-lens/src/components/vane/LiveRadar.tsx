...
export const LiveRadar: React.FC<LiveRadarProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-[#21262d] bg-[#121824] shadow-2xl p-5 space-y-5 select-none animate-fadeIn">
      ...
      <div className="flex items-center justify-between border-b border-[#21262d] pb-4">
          ...
          <div>
            <h3 className="text-xs font-bold text-slate-100 flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${
                loading ? 'bg-sky-500/20 text-sky-400 border border-sky-500/30 animate-pulse' : 'bg-emerald-500/20 text-emerald-400'
              }`}>
        ...
        <div className="flex items-center gap-2">
          <div className="px-3 py-1.5 rounded-xl bg-[#0c1017] border border-[#21262d] text-right rtl:text-left">
            <span className="block text-[10px] text-slate-400 font-mono">{isArabic ? 'المصادر المستخرجة' : 'Sources'}</span>
            <span className="text-xs font-bold text-sky-400 font-mono">{sources.length}</span>
          </div>
          <div className="px-3 py-1.5 rounded-xl bg-[#0c1017] border border-[#21262d] text-right rtl:text-left">
            <span className="block text-[10px] text-slate-400 font-mono">{isArabic ? 'خطوات التفكير' : 'Steps'}</span>
            <span className="text-xs font-bold text-purple-400 font-mono">{steps.length}</span>
          </div>
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`p-2.5 rounded-xl border text-center space-y-1 transition ${
                  isCurrent 
                    ? 'bg-sky-500/10 border-sky-500/50 shadow-[0_0_12px_rgba(56,189,248,0.15)] ring-1 ring-sky-500/30' 
                    : isCompleted 
                    ? 'bg-[#161b22] border-white/5 text-slate-300' 
                    : 'bg-[#0c1017]/40 border-white/5 text-slate-600 opacity-60'
                }`}
              >
      ...
      <div className="rounded-xl border border-[#21262d] bg-[#0c1017] overflow-hidden">
        <button
          type="button"
          onClick={() => setShowTerminal(!showTerminal)}
          className="w-full px-4 py-2 bg-[#161b22] flex items-center justify-between text-xs text-slate-400 hover:text-slate-200 transition"
        >
          <div className="flex items-center gap-2">
        ...
        {showTerminal && (
          <div className="p-3 max-h-48 overflow-y-auto space-y-2 font-mono text-[11px] custom-scrollbar bg-[#07090e]/90 text-slate-300">
            {steps.length === 0 ? (
