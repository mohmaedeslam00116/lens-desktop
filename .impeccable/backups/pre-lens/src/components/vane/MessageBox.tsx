    ...
    <div className="w-full max-w-4xl mx-auto px-4 py-8 space-y-7 animate-fadeIn">
      ...
      <div className="space-y-2 pb-2">
        <div className="flex items-center gap-2 text-xs font-mono text-[#2dd4bf]">
          ...
          <span>{isArabic ? 'استعلام البحث والتحليل العميق' : 'Autonomous Deep Research'}</span>
        </div>
            ...
            <div className="flex items-center gap-1 p-1 bg-white/[0.03] border border-white/[0.06] rounded-xl">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                  viewMode === 'report' 
                    ? 'bg-white/[0.08] text-[#2dd4bf] shadow-sm' 
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              {'speechSynthesis' in window && (
                <button
                  ...
                  className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-xs font-medium transition ${
                    isSpeaking 
                      ? 'bg-[#2dd4bf]/10 text-[#2dd4bf] border-[#2dd4bf]/30 animate-pulse' 
              ...
              {extractedTables.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-xs font-medium transition"
                  title="تصدير جداول البيانات كـ CSV"
                >
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <Icon className="w-3.5 h-3.5 text-[#2dd4bf] shrink-0 opacity-80" />
                    <span className="truncate font-medium">{pill.label}</span>
                  </div>
                  <Plus className="w-3.5 h-3.5 text-slate-500 group-hover:text-[#2dd4bf] shrink-0 transition-colors" />
                </button>
              );
            })}
          </div>
