...
export const DiscoverView: React.FC<DiscoverViewProps> = ({ onSelectTopic, language }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-8 space-y-8 animate-fadeIn select-none">
      {/* 1. Header Section */}
      <div className="space-y-3 pb-6 border-b border-white/[0.06]">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2 text-xs font-mono text-[#2dd4bf]">
            ...
            <span>{isArabic ? 'رادار الاستكشاف الإخباري الحي' : 'Live Intelligence Feed'}</span>
          </div>
          ...
          >
            <RefreshCw className={`w-3 h-3 text-[#2dd4bf] ${loading ? 'animate-spin' : ''}`} />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-100 tracking-tight">
              {isArabic ? 'استكشف أحدث المستجدات والاتجاهات' : 'Discover Global Trends & News'}
            </h1>
          ...
          <form onSubmit={handleSearchSubmit} className="relative min-w-[260px] sm:w-72">
            <input
              type="text"
              value={searchFilter}
              onChange={(e) => setSearchFilter(e.target.value)}
              placeholder={isArabic ? 'ابحث في الأخبار الحية...' : 'Search live news...'}
              className="w-full pl-9 pr-3 rtl:pr-9 rtl:pl-3 py-2 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] focus:bg-[#111620] border border-white/[0.07] focus:border-[#2dd4bf]/40 focus:ring-1 focus:ring-[#2dd4bf]/20 text-xs text-slate-200 placeholder-slate-500 transition focus:outline-none"
            />
      ...
      {!loading && error && articles.length === 0 && (
        <div className="p-8 rounded-2xl bg-white/[0.02] border border-white/[0.05] text-center space-y-3">
          ...
          <button
            type="button"
            onClick={() => fetchLiveNews(activeTopic, searchFilter)}
            className="px-4 py-2 rounded-xl bg-[#2dd4bf] text-slate-950 text-xs font-semibold hover:bg-[#2dd4bf]/90 transition"
          >
        ...
        <div className="space-y-6">
          ...
          {heroArticle && (
            <div 
              ...
              className="p-6 rounded-2xl bg-gradient-to-br from-white/[0.04] via-white/[0.02] to-transparent hover:from-white/[0.07] hover:to-white/[0.02] border border-white/[0.07] hover:border-[#2dd4bf]/40 cursor-pointer transition-all duration-200 group shadow-lg shadow-black/20 relative overflow-hidden"
            >
              {/* Subtle ambient light */}
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#2dd4bf]/5 rounded-full blur-3xl pointer-events-none group-hover:bg-[#2dd4bf]/10 transition-all" />
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="flex items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="font-medium text-[#2dd4bf]">{heroArticle.source}</span>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/[0.05] text-xs">
                  <div className="flex items-center gap-1.5 text-[#2dd4bf] font-medium group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 transition">
            ...
            {gridArticles.map((art) => (
              ...
              >
                <div className="space-y-2">
                  ...
                  {/* Title */}
                  <h3 className="text-xs sm:text-[13px] font-semibold text-slate-200 group-hover:text-[#2dd4bf] transition leading-snug line-clamp-3">
                    {art.title}
                  </h3>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/[0.04] text-[11px] text-slate-500">
                  <span className="flex items-center gap-1 group-hover:text-[#2dd4bf] transition font-medium">
