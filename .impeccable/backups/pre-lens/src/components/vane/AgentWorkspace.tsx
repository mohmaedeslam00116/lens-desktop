...
export const AgentWorkspace: React.FC<AgentWorkspaceProps> = ({
  ...
  return (
    <div className="space-y-6 select-text animate-fadeIn">
      {/* Workspace Banner */}
      <div className="p-4 rounded-2xl bg-gradient-to-r from-[#121824] to-[#161b22] border border-[#21262d] flex items-center justify-between gap-4 select-none">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-sky-500/20 text-sky-400 border border-sky-500/30">
                Autonomous Assets
              </span>
            </h3>
      ...
      {/* Navigation Sub-Tabs */}
      <div className="flex items-center gap-2 border-b border-[#21262d] pb-2 overflow-x-auto select-none">
          ...
          {tables.length > 1 && (
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full bg-[#121824] border border-[#21262d] rounded-xl pl-8 pr-3 rtl:pl-3 rtl:pr-8 py-2 text-slate-200 text-xs focus:outline-none focus:border-sky-500/50"
              />
            </div>
          )}

          {filteredTables.length === 0 ? (
            <div className="py-12 text-center rounded-2xl border border-dashed border-[#21262d] bg-[#121824]/40 space-y-2">
            ...
            filteredTables.map((tbl, idx) => (
              <div 
                key={tbl.id}
                className="rounded-2xl border border-[#21262d] bg-[#121824] shadow-xl overflow-hidden space-y-0 transition hover:border-[#30363d]"
              >
                {/* Table Header Bar */}
                <div className="p-4 bg-[#161b22] border-b border-[#21262d] flex flex-wrap items-center justify-between gap-3">
          ...
          {metrics.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {metrics.map((m, idx) => (
                <div 
                  key={idx}
                  className="p-4 rounded-xl bg-[#121824] border border-[#21262d] hover:border-sky-500/40 transition space-y-1.5 shadow-sm"
                >
                  <p className="text-[11px] font-semibold text-slate-400 truncate">{m.label}</p>
                  <p className="text-sm font-bold text-sky-400 font-mono leading-snug">{m.value}</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {sources.map((s, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="p-3.5 rounded-xl bg-[#121824] border border-[#21262d] hover:border-sky-500/30 transition flex flex-col justify-between gap-2.5"
                >
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="w-5 h-5 rounded-md bg-[#161b22] border border-[#30363d] text-sky-400 flex items-center justify-center text-[10px] font-mono font-bold shrink-0">
                          {idx + 1}
                        </span>
                        <img 
                          src={faviconUrl} 
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-[10px] text-sky-400 hover:text-sky-300 transition font-medium self-end"
                  >
