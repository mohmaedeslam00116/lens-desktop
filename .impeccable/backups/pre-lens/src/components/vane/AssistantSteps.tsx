...
export const AssistantSteps: React.FC<AssistantStepsProps> = ({ steps, loading, language }) => {
  ...
  if (!steps || steps.length === 0) {
    ...
    return (
      <div className="flex items-center gap-2.5 p-3 rounded-xl bg-[#161b22] border border-[#21262d] text-xs text-slate-400">
  ...
  return (
    <div className="w-full rounded-xl bg-[#161b22] border border-[#21262d] overflow-hidden transition-all duration-200">
      <button
        type="button"
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full p-3 flex items-center justify-between text-left rtl:text-right hover:bg-[#21262d]/50 transition"
      >
      ...
      {isExpanded && (
        <div className="px-4 pb-3 pt-1 border-t border-[#21262d]/60 space-y-2">
          {steps.map((step, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-2.5 text-xs">
                ...
                <div className="min-w-0">
                  <p className={`leading-relaxed ${isLast && loading ? 'text-sky-300 font-medium' : isThinkingStep ? 'text-amber-200/90' : 'text-slate-300'}`}>
                    {step.step}
                  </p>
                  {step.details && (
