...
["query 1", "query 2", "query 3"]`;
    ...
    this.emitEvent({
      type: 'status',
      message: 'جاري صياغة التقرير النهائي وتنظيم الاستشهادات المرجعية...',
      step: 'synthesizing'
    });

    this.emitEvent({
      type: 'thought',
      thought: 'توليف الأدلة المعرفية وصياغة فصول التقرير بأسلوب تحليلي موثق بأرقام المراجع [1] [2]...'
    });

    // Build context with numbered citations [1], [2], ...
    const evidenceText = scrapedSources.map((s, idx) => {
