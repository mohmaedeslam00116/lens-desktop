...
### Added
- **Frontend (issue #121 — LENS harness workspace)**: added an opt-in desktop workspace preview based on the user-authored Stitch shell. It provides a useful research-history rail, a focused bilingual question/event canvas, live researcher and query activity, and a conditional plan/evidence/report/graph inspector. The preview reuses existing LENS session state and callbacks, preserves the default workspace and routes, and marks transport retries and terminal failures without fabricating agent or developer-workflow data.
...
ADR-0010 research-agency architecture (phases 1–4: parent/researcher
orchestration, fan-out, auditor, roles, parity harness, agency default flip,
legacy-loop removal) and phase 5's primary-plane swap (ADR-0013): pi-web-access
is now the **sole retrieval plane** — search, scrape, keyed, and keyless —
behind LENS's seams, with evidence admission, dedupe, budgets, telemetry
boundaries, and evidence preservation remaining LENS-authoritative and every
swap step gated by the parity harness (ADR-0011).

### Added
- **Engine (ADR-0013 config seam — ticket #112, D2 complete)**: LENS settings now write through to the pi-web-access config path (`engine/configSeam.ts` → `web-search.json`), making keyed providers (Tavily/Serper) opt-in — merge-on-write preserves vendored-managed fields (`ssrf`, `fetch`, …), clearing a key removes the field and its environment export, and with no keys configured NO file is written (zero-config parity pinned by test). The seam exports `TAVILY_API_KEY`/`SERPER_API_KEY` in-process for immediate effect (the vendored resolver reads env per call, so fresh keys work without restart) while the config file remains the durable copy (vendored precedence: file > env). The keyed provider HTTP clients retired: keyed search serves through the vendored `searchWithTavily`/`searchWithSerper` modules under the same bounded gate and ledger as the keyless plane, `MultiSearchProvider.search` reduces to a plane delegation, and a keyed failure degrades to the keyless DDG plane (terminal in both directions — no cycle). No-leak clause enforced: the new `/api/settings/search-keys` endpoint (POST write-through, GET boolean status) returns only redacted summaries — keys are never echoed, logged, or sent to telemetry; plane warn paths pass through `redactKeyMaterial`. Settings-save fires the write-through fire-and-forget (engine unreachable → keys persist in LENS settings only). Full suite green (444/444).

- **Engine (ADR-0013 scrape-plane increment — ticket #111)**: page retrieval moves onto the primary plane — pi-web-access `extractContent` (the engine of `fetch_content`) now serves scrapes behind the engine's scrape seam via `engine/scrapePlane.ts`, and the native fetch/cheerio scraper is retired (`PageScraper.scrape` remains as a thin facade over `primaryScrapePlane`, preserving the seam tests stub to inject fixtures; the delegated loop, researchers, and `BoundedScraperPool` route through the plane unchanged). Boundary clause D5 holds: every fetch is admitted through a bounded, abort-aware concurrency gate (waiter slot inheritance, saturation fails loudly) and counted in the scrape-plane ledger (`scrapePlaneLedgerSnapshot`) — no unledgered retrieval. Vendored SSRF validation stays ON (private/loopback addresses blocked pre-fetch — hardening over the native scraper); origin HTTP errors map to the native `Content unavailable from … (HTTP N).` sentinel the pool's backoff keys on, transport failures throw the native `Failed to scrape …` error, caller aborts propagate as `AbortError`, and vendored timeouts map to the native `Request timed out`; LENS-owned credibility provenance and the ~6,000-char content budget are applied LENS-side. The parity harness now proves fixture runs scrape through the vendored plane (scrape-plane ledger gate), its runner resolves fixture hostnames offline through the plane's lookup seam (non-fixture hostnames fail loudly), and a latent #109 bug it exposed is fixed: the DDG route matcher decoded only the native `%20` wire form, so the vendored plane's `+`-encoded queries 404'd silently and the parity legs passed on empty results — the matcher now routes both forms, and the legs run on real golden content. Full suite green (437/437).

- **Engine (ADR-0010 phase 5 — ticket #109)**: pi-web-access is now the **primary retrieval plane** (ADR-0013) — engine search resolves through a new seam adapter (`engine/searchPlane.ts`) that drives the vendored DuckDuckGo provider behind the engine's existing `searchFn` seam, so the delegated loop, researchers, and the parity harness run unchanged while what sits behind the seam changes. Every vendored-plane call is admitted through a bounded concurrency gate and counted in the plane ledger (`searchPlaneLedgerSnapshot`) — no unledgered retrieval (non-negotiable boundary clause). Keyed providers (Tavily/Serper keys present) keep the native path until the config seam (#112 — intermediate migration state, superseded later in this same release by #112's vendored keyed path); any vendored-plane failure falls back through the canonical native seam entry (`MultiSearchProvider.search`) with caller aborts propagating. `fetch_content` `mode: 'answer'` is explicitly unsupported (ADR-0013 D3): the package tool handler intercepts it pre-dispatch with graceful guidance — synthesis stays exclusively Parent-owned, and pi-web-access makes zero LLM calls under LENS. The parity harness now proves fixture runs exercise the vendored plane (plane-ledger gate assertion) with all four stage semantics strict. Full suite green (427/427).

- **Engine (ADR-0010, agency phase 3 — ticket #94)**: parity regression harness (`engine/parityHarness.ts`, ADR-0011) — the gate that protects the agency default flip, auditor gating, and fan-out constant tuning. It replays golden offline fixture runs through the legacy standard loop and BOTH agency paths (delegation leg + researcher fan-out leg) in one process, serving deterministic fixture HTML to the REAL retrieval stack (MultiSearchProvider + PageScraper + DeduplicationEngine + BoundedScraperPool — no network), and diffs four documented stages: coverage score (|Δ| ≤ 0.05, order-insensitive admission semantics), citation-grounding audit (exact sanitizer decisions over the shared evidence pool), admission sets (exact distinct-URL sets), and the shared LiveEvent backbone (delegation vs legacy, the #88 byte-equivalence contract generalized). Divergence beyond thresholds fails with a JSON diff artifact + human-readable report identifying the fixture, stage, expected, and actual; threshold overrides let tests force divergence to prove the artifact path. Two golden fixtures (EN) pin baseline parity; fixture scope documented (quick depth: ≤ 2 facets × ≤ 2 pages). Full suite green (420/420).
- **Engine (ADR-0010, agency phase 3 — ticket #93)**: closed Researcher Role Catalog with deficit-driven re-specialization. Facets deterministically select from five roles (`primary`, `technical`, `opposing`, `recent_news`, `source_verifier`) via keyword scoring over the facet query (`engine/researcherRoles.ts`), each role carrying a specialized researcher brief woven into the facet's retrieval prompt; selection streams as an additive `role_selected` `researcher_telemetry` phase (before the `started` lifecycle). After the fan-out, per-facet coverage (via the authoritative `auditEvidenceCoverage`) drives budget-bounded re-specialization: up to 4 follow-up researchers per run launch against deficit facets (below 0.5 or unmeasured), with the follow-up role mapped deterministically from the deficit kind (zero-coverage primary → technical deep-dive; partial technical/verifier → source_verifier; etc.), every launch re-checked against the single session budget, and follow-up findings merged into the facet's seed with fresh provenance. Respecialization respects the parent's clamped concurrency limit through the same worker pool and remains opt-out-able per run (`{ respecialization: false }`) for deterministic fixture runs. Full suite green (417/417).
- **Engine (ADR-0010 — ticket #92)**: compression routing for researcher long-context traffic. With `compression_mode` (requires `researcher_mode`, dormant by default), the parent acquires ONE supervised billion-context proxy lease per run and passes its URL to every researcher, so `generate()` requests carry the proxy base URL and the pi adapter rewrites each model's upstream `baseUrl` through the bili URL-prefix contract (`http://127.0.0.1:<port>/bili/<upstream>` — upstream preserved in the path, no MITM/CA); the lease is released in a `finally` after the fan-out so the child never outlives the run. Any failure mode — missing binary, startup timeout, unhealthy health probe, abort during startup — degrades gracefully to the uncompressed run with a telemetry notice; compression is an optimization, never a correctness gate. The supervisor's health probe runs over raw `node:http` (immune to app-level fetch instrumentation), honors AbortSignals end-to-end, and its dispose path reaps the child (identity-checked against pid reuse, error-event safe); the binary path supports a `LENS_BILI_BINARY` override. Tests run a stub child binary through the real supervisor seam with free-port allocation — no bundled billion-context startup in-process, no real network. Full suite green (406/406).
- **Engine (ADR-0010, agency phase 3 — ticket #91)**: advisory Evidence Auditor. After retrieval and synthesis, the engine audits the report's important claims against the retrieved evidence (`engine/evidenceAuditor.ts`) — deterministic, offline, bilingual claim-vs-evidence verification reusing the coverage audit's tokenizer (Arabic normalization + light stemming), with bounded claim extraction (≤ 40) and thresholded verdicts (`supported` / `partially_supported` / `unsupported`, plus per-claim support ratio and best-evidence pointer). Verdicts stream as the additive `audit_telemetry` LiveEvent (mirrored in the renderer's `LiveEvent` type) and the report itself surfaces the outcome via a bilingual `## Evidence Audit` / `## تدقيق الأدلة` section appended as the final streamed chunk — streamed-vs-final report equality preserved. **Advisory only** per ADR-0010 decision 4: verdicts never gate admission, coverage, early exit, or budget decisions (gating is a settings flag reserved for post-#94 parity data). Full suite green (396/396).
- **Engine (ADR-0010, agency phase 3 — ticket #90)**: parallel facet fan-out for multi-facet agency plans. Researchers now run concurrently under an explicit cap (`min(#facets, 4)` per ADR-0010 decision 8, optionally overridden via the clamped `researcher_concurrency` request field) through a bounded worker pool; cross-researcher URL dedupe is enforced by a new session-scoped fetch ledger (`engine/fetchLedger.ts`) anchored on the LENS canonical-URL contract — a URL fetched by one researcher is never re-fetched by another (concurrent callers share the identical admitted page, failed fetches release the claim for retry), with the dedupe yield surfaced per researcher (`dedupeShared` telemetry) and aggregated in the new additive `fanout_telemetry` LiveEvent (concurrency/budget limits, launched/completed/failed researchers, delegated facets, shared URLs, session budget state, and per-facet coverage aggregation via the authoritative `auditEvidenceCoverage`). Session caps (`8` researchers / `48` findings) gate every launch with per-launch budget re-checks, overflow facets falling back to the delegated loop; ledger and budget state evict on session teardown. Full suite green (389/389).
- **Engine (ADR-0010, agency phase 2 — ticket #89)**: single in-process researcher subagent. With `researcher_mode` (requires `agency_mode`, both default off), each facet from the approved plan executes inside a `ResearcherAgent` — a scoped pi-core run over the LENS retrieval backbone (`MultiSearchProvider` + `PageScraper`, budget-bounded) with the vendored pi-web-access toolset (web_search/source_check/fetch_content/get_search_content) attached as the supplementary model-driven plane; tool-surfaced URLs are harvested into the facet's evidence pool. Every finding is tagged with the facet's milestone provenance (`milestoneId`/`milestoneTitle` on `ScrapedPage`, `source` LiveEvents, and finished-report sources), researcher progress streams as additive `researcher_telemetry` phases (`started`/`retrieval`/`tool_activity`/`completed`), and seeded evidence flows into the delegated loop's pool without breaking citation grounding or the parity contract. Researcher construction goes through a parent-side factory seam (pi-subagents-shaped) for later phases and offline tests. Full suite green (383/383).
- **Engine (ADR-0010, agency phase 1 — ticket #88)**: Parent Research Agent orchestration seam behind the dormant `agency_mode` request flag (default off — unflagged runs are unchanged). The parent derives research facets 1:1 from the approved plan (milestone-to-facet pass-through; role catalog lands with #93), maintains the session-scoped rpiv-todo research plan through parent-only direct store writes (renderer sees read-only projections only, ADR-0010 decision 7), and emits the additive `researcher_telemetry` LiveEvent (started/completed lifecycle with `researcherId`/`role`/`facet`/phase/size-capped todo-projection (truncation flagged via `todoProjectionTruncated`), following the `wide_telemetry` precedent) while evidence keeps flowing as ordinary `source` events. Retrieval and synthesis are delegated verbatim to the legacy standard loop, so the final report is byte-equivalent by construction — verified by a faux-provider parity test (assembled stream and `finished` event byte-identical; event backbone equal after collapsing arrival-timing-dependent chunk boundaries). Also: parent-only `loadTodoPlanStore` accessor over the rpiv-todo pure state modules in the packages bridge, and the additive event mirrored in the renderer's `LiveEvent` type. Full suite green (376/376).
- **Engine (ADR-0009, package integration)**: in-process bridge for the vendored pi ecosystem packages inside the Electron main process — `pi-web-access` loads through jiti and registers its four research tools (`web_search`, `source_check`, `fetch_content`, `get_search_content`), and `@juicesharp/rpiv-todo` provides the `todo` tool through its pure `state/`+`tool/` modules (session-scoped store, reducer, snapshot-in-`details` envelope) — vendored verbatim to `dist-electron/vendor/pi/` by `scripts/vendor-pi-packages.mjs` (ESM-marked copies, jiti-compatible). `pi-subagents` remains deferred (needs a real pi-coding-agent extension host; decision recorded in `docs/research/responsibility-mapping.md`), and `billion-context` stays out-of-process behind the supervised-subprocess supervisor seam (`engine/billionContext.ts`) — never imported in-process, since its bundle self-starts a server on import. All attached to the agent loop behind the opt-in `tool_packages` request flag (default off, no behavior change). Full suite green (370/370).
...
### Changed
- **Engine (ADR-0010 phase 4, contract step — ticket #95)**: research runs now DEFAULT to the agency path (ADR-0012). The standard loop routes through the Parent Research Agent (parallel role-specialized researchers under the session budget, advisory evidence audit, compression routing); the legacy single-loop remains reachable via the new `legacy_mode: true` escape flag — request flag wins over the settings default (`legacyMode` toggle in Research settings, bilingual UI) — pending removal (expand–contract closure, follow-up ticket #104; intermediate migration state — the flag is removed later in this same release, see Removed). Wide mode is never rerouted. The authorization invariant is unchanged: retrieval is strictly gated until the plan is approved — the session lifecycle's approval gate precedes any agent construction, and the parent's engine guard rejects unapproved plans regardless of routing. Renderer payload contract unchanged (the flip is server-side routing). Full suite green (423/423).
...
- **Engine (ADR-0009, dormant seam work)**: added the pi-ai provider adapter implementing the existing model request contract (`LLMRequestOptions`) — 7 native cloud providers + Ollama via a custom OpenAI-compatible provider, token streaming, depth-capped tool loop, abort support — plus the pi-agent→`LiveEvent` event adapter mapping, and the Skills bridge registering `activate_skill` as the single pi tool (skills pipeline + sandboxing unchanged behind it). All dormant behind the seam: no agent loop uses them yet; an ESM shim bridges the ESM-only pi packages into the CJS engine build. Full suite green (363/363) (tickets #68, #69, #70).
- **Engine (ADR-0009, loop migration)**: introduced the AgentCore generation gateway (`generate()`) and migrated all research-loop LLM call sites (standard loop: subqueries/report/followup; hierarchical synthesis: milestone/meta passes) onto it. The active core defaults to the legacy client; switching to the pi core routes every loop call through the pi-backed adapter — verified end-to-end with faux providers (standard LiveEvent stream parity incl. approved-plan frozen trajectory, wide synthesis with citation grounding intact, abort fallback). Full suite green (368/368) (tickets #71, #72).
- **Engine (ADR-0009, retirement)**: retired the hand-rolled multi-provider LLM plumbing — per-provider SSE/tool-call parsing (now a fail-loud stub), provider tool shaping (`formatProviderTools`), and `ModelClient.generate` with its three private provider implementations (~465 lines removed). `modelGateway` is now pi-only (the legacy core switch throws), settings model catalogs (`fetchDynamicModels`) and connection tests (routed through the gateway) are preserved. Full suite green (365/365) (ticket #73). CodeRabbit hardening on the same PR: bounded 10s timeout on the gateway connection test, `wirePiEvents` now returns the SDK disposer with an idempotent active-flag guard (regression tests added), and the prototype workstation uses the `dir` attribute (not `rtl`/`ltr` classes) for Arabic layout. Full suite green (367/367).

### Removed
- **Engine (ADR-0013 contract step — ticket #110)**: the LENS-native DuckDuckGo search implementation (~135 lines: `searchDuckDuckGo` HTML fetch/parser + the unused `searchDuckDuckGoInstantApi` Instant-Answer module) — the vendored pi-web-access plane (searchPlane.ts, #109) is the **only** keyless search path; retrieval no longer has a second implementation. The native `search()` seam remains as the keyed-provider entry (Tavily/Serper stay until the #112 config seam), with its keyless fallback routed to the primary plane; the plane's own failure fallback goes straight to the caller's keyed provider (terminal — the plane↔native re-entry cycle was caught and removed during implementation, surfaced by a hung suite: 39k fallback iterations). The wide-mode abort contract (prompt AbortError on stalled bodies, endpoint override) is re-pinned on the vendored plane via module-level test seams (`__testSeams.searchWithDuckDuckGo`) with the process fetch untouched. No soak per operator decision D8 (engine is the sole consumer; the parity harness is the gate). Full suite green (430/430).

- **Engine (ADR-0010 closure, contract step — ticket #104)**: the legacy single-loop research route and its escape flag. `createResearchAgent` reduces to a two-way split (standard → Parent Research Agent, wide → WideResearchAgent); the `legacy_mode` request flag and the `legacyMode` settings toggle (`researchDefaults.ts` deleted) are removed — requests still carrying the flag have it silently dropped (no schema validation; deprecation-error alternative rejected as client-breaking, decision recorded on #104 and in ADR-0012's amendment). The soak criterion was waived by the operator (no field-telemetry channel exists; the waiver + CI-gate rationale is recorded on #104). `DeepResearchAgent` survives only as the parent's delegated implementation (plus the stateless followup-Q&A static and the #88 agent-level parity pin). The #94 parity harness runs two agency legs (delegation baseline vs fan-out). Full suite green (422/422).
...
- **Full CodeRabbit Audit Remediation across Embedded Engine and Frontend**:
  - **Embedded Engine & Network Hardening**:
    ...
    - Cancelled oversized response bodies before rejecting their declared content lengths.
    ...
    - Retried failed embedding index flushes without marking newly cached vectors clean.
    - Terminated retained WebSockets before releasing completed research sessions.
    ...
    - Read skill resources through one no-follow descriptor with bounded, abortable streaming.
  - **React Frontend**:
    ...
    - Bounded dropped-directory traversal by entry count and decoded content bytes.
    - Ignored stale WebSocket events using per-session generation tokens.
