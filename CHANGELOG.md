...
### Changed
- **Engine (ADR-0010 phase 4, contract step — ticket #95)**: research runs now DEFAULT to the agency path (ADR-0012). The standard loop routes through the Parent Research Agent (parallel role-specialized researchers under the session budget, advisory evidence audit, compression routing); the legacy single-loop remains reachable via the new `legacy_mode: true` escape flag — request flag wins over the settings default (`legacyMode` toggle in Research settings, bilingual UI) — pending removal (expand–contract closure, follow-up ticket). Wide mode is never rerouted. Plan-less default runs degrade gracefully: the parent logs a warning, emits a telemetry notice, and delegates to the legacy loop with all seams attached (an EXPLICIT `agency_mode: true` request without an approved plan still fails loudly). Renderer payload contract unchanged (the flip is server-side routing). Full suite green (423/423).
