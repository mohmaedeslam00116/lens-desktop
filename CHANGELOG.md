...
### Added
- **Engine (ADR-0010 — ticket #92)**: compression routing for researcher long-context traffic. With `compression_mode` (requires `researcher_mode`, dormant by default), researchers resolve the supervised billion-context proxy before their first model call and `generate()` requests carry the proxy base URL, so the pi adapter rewrites each model's upstream `baseUrl` through the bili URL-prefix contract (`http://127.0.0.1:<port>/bili/<upstream>` — upstream preserved in the path, no MITM/CA). Any failure mode — missing binary, startup timeout, unhealthy health probe — degrades gracefully to the uncompressed run with a telemetry notice; compression is an optimization, never a correctness gate. The supervisor's dispose path reaps the child (identity-checked against pid reuse, error-event safe). Tests run a stub child binary through the real supervisor seam — no bundled billion-context startup in-process, no real network. Full suite green (406/406).
