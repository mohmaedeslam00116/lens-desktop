...
## Decision
We enforce a mandatory collaborative scoping phase (`Phase 1`) only for explicitly selected wide research sessions (`ResearchMode = 'wide'`). In this phase:
...
3. Upon approval, the approved milestones freeze the retrieval trajectory: `WideResearchAgent` executes search directly across the approved milestone queries, preventing independent query drift.
